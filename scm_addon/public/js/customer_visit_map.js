// Customer Visit Map - Using Leaflet
function initCustomerVisitMap() {
    ...
    function initMap() {
        ...
        // Create map centered on Kenya (default location)
    ...
    function renderMarkers(visits) {
        ...
        visits.forEach(function(visit) {
            if (visit.start_latitude && visit.start_longitude) {
                const key = visit.customer;
                if (!visitsByCustomer[key]) {
                    visitsByCustomer[key] = [];
                }
                visitsByCustomer[key].push(visit);
                ...
                const popupContent = `
                    <div style="min-width: 250px; font-size: 12px;">
                        <strong style="color: #333;">${visit.customer}</strong><br>
                        <small>Visit ID: ${visit.name}</small><br>
                        <small>Date: ${frappe.format(visit.date, {fieldtype: 'Date'})}</small><br>
                        <small>Sales Person: ${visit.sales_person || 'N/A'}</small><br>
                        ...
                        <hr style="margin: 8px 0;">
                        <a class="btn btn-sm" href="/app/customer-visit/${visit.name}" target="_blank" style="display: inline-block;">View Details</a>
                    </div>
                `;
                ...
                if (visit.stop_latitude && visit.stop_longitude) {
                    ...
                    const stopPopupContent = `
                        <div style="min-width: 250px; font-size: 12px;">
                            <strong>${visit.customer} (Stop)</strong><br>
                            <small>Visit ID: ${visit.name}</small><br>
    ...
    function addStatistics(visits) {
        const stats = {
            ...
            unique_sales_people: new Set(visits.map(v => v.sales_person)).size
        };
...
// Auto-initialize if Frappe is ready
